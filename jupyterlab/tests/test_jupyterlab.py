...
class TestExtension(AppHandlerTest):
    ...
    def test_disable_extension(self):
        ...
        assert disable_extension('@evolab/notebook-extension', app_options=options) is True
        info = get_app_info(app_options=options)
        assert '@evolab/notebook-extension' in info['disabled']
        assert not check_extension('@evolab/notebook-extension', app_options=options)
        assert check_extension('@evolab/notebook-extension', installed=True, app_options=options)
        assert name in info['disabled']
        assert not check_extension(name, app_options=options)
        assert check_extension(name, installed=True, app_options=options)
    ...
    def test_enable_extension(self):
        ...
        assert name not in info['disabled']
        assert check_extension(name, app_options=options)
        assert '@evolab/notebook-extension' not in info['disabled']
        assert not check_extension('@evolab/notebook-extension', app_options=options)
