...
import { CommHandler,exec_handler } from './comm';

import * as Kernel from './kernel';

import * as KernelMessage from './messages';

// import { NotebookActions } from '@evolab/notebook';
...
// import {IExecuteResultMsg} from "./messages";
// import {IStreamMsg} from "./messages";
...
export class KernelConnection implements Kernel.IKernelConnection {
  ...
  constructor(options: Kernel.IKernelConnection.IOptions) {
    ...
    this.count_cell=[];
    this.multi_runcell=[];
    this.exechandler=new exec_handler();
  ...
  // private _sendMessage(msg: KernelMessage.IMessage, queue = true) {
    private _sendMessage(msg: KernelMessage.IMessage, queue = true) {
    console.log('_send websocket Message');
    if (this.status === 'dead') {
    ...
    if (
      ...
      console.log(serialize.serialize(msg));
   ...
   */
  //处理自定义msg，这里修改为收到信息后进行cell内容的同步
  private async _handleCommMsg(msg: KernelMessage.ICommMsgMsg): Promise<void> {
    this._assertCurrentMessage(msg);
    if(msg.content.data.func==="sync" && (msg.content.comm_id!==this._commid ||msg.content.data.spec=="lockcell" || msg.content.data.spec=="unlockcell")){
      console.log(this._commid);
      const comm = this._comms.get(this._commid);
      if (!comm) {
        return;
      }
      console.log('get comm by id');
      //设置onMsg
    ...
    //   }
    // }
    // const cell_id = content.data['id'];
    // console.log(cell_id);
    // NotebookActions.SetCellById(notebook,cell_id,content.data['txt']);
    // cell.model.value.text=content.data['txt']
    //根据cell_id得到对应的cell
  ...
  private _createSocket = () => {
    ...
    if (token !== '') {
      url = url + `&token=${encodeURIComponent(token)}`+`user=`+this.getCookie("username");
    }
  ...
  private async _handleMessage(msg: KernelMessage.IMessage): Promise<void> {
    ...
    if(this.on_execute &&
      ...
      if(msg.parent_header.hasOwnProperty('session')){
        // @ts-ignore
        if( msg.parent_header.session !== this.clientId){
          console.log("on execute****************");
        // tslint:disable-next-line:await-promise
        // const onExecMsg = this.exechandler._onExecMsg()onExecMsg;
          // tslint:disable-next-line:await-promise
        await this.exechandler._onExecMsg(msg);
          return
        }
      }
    }
