...
export namespace NotebookActions {
  ...
  export function mergeCells(notebook: Notebook): void {
    console.warn("merge cell!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    console.debug("mergecell")
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
   ...
   * This action can be undone.
   */
  export function deleteCells(notebook: Notebook,comm:Kernel.IComm | null): void {
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
    ...
    Private.deleteCells(notebook,comm);
   ...
   * @param comm
   */
  export function insertAbove(notebook: Notebook,comm:Kernel.IComm | null): void {
    console.debug('insertabove');
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
    const id=UUID.uuid4();
    ...
    model.cells.insert(active, cell);
     const sendx={'cellid':id,'index':notebook.activeCellIndex ,'func':'sync','spec':'createcell'};
    comm!.send(sendx);
  ...
  export function insertBelow(notebook: Notebook, cellType:nbformat.CellType="code", comm:Kernel.IComm | null): void {
    ...
    const sendx={'cellid':id,"celltype":cellType,'index':notebook.activeCellIndex + 1,'func':'sync','spec':'createcell'};
    comm!.send(sendx);
    // Make the newly inserted cell active.
  ...
  // export function GetCellById(notebook:Notebook,id:string,txt:string):void {
  //   const cell=notebook.widgets.find(x => x.model.id === cellmodel.id);
  //   cell.model.value.txt=txt
  // }
  ...
  export function changeCellType(
    ...
    console.debug('cell type change');
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
    ...
    Private.changeCellType(notebook, value,comm);
    Private.handleState(notebook, state);
  }
  ...
  export function run(
    ...
    const state = Private.getState(notebook);
    const promise = Private.runSelected(comm,notebook, sessionContext);
  ...
  export function runAndAdvance(
    ...
    if (notebook.activeCellIndex === notebook.widgets.length - 1) {
      ...
      const sendx={'cellid':cellid,'index':notebook.activeCellIndex + 1,'func':'sync','spec':'createcell'};
      comm!.send(sendx);
      notebook.activeCellIndex++;
      notebook.mode = 'edit';
    } else {
   ...
   * @param notebook - The target notebook widget.
   */
  export function copy(notebook: Notebook,comm:Kernel.IComm | null): void {
    Private.copyOrCut(notebook, false,comm);
  }
   ...
   */
  export function cut(notebook: Notebook,comm:Kernel.IComm | null): void {
    Private.copyOrCut(notebook, true,comm);
  }
   ...
   */
  export function setMarkdownHeader(notebook: Notebook, level: number,comm:Kernel.IComm | null) {
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
    ...
    Private.changeCellType(notebook, 'markdown',comm);
    Private.handleState(notebook, state);
  }
...
export namespace Private {
  ...
  export function runSelected(
    ...
    if(codecell_length !== selected.length && codecell_length !==0){
      console.log("executed cell has readonly");
      return null
    }else if(codecell_length == selected.length ){
      cellListId=UUID.uuid4();
      const sendx={"cellid":"n",'cellListId':cellListId,'username':getCookie('username'),'func':'lock_list',"cellnum":codecell_length};
      comm!.send(sendx);
    }
    ...
    if(cell.readOnly==true){
      ...
      switch (cell.model.type) {
        case 'markdown':
          ...
          const sendx={'cellid':cell.model.metadata.get("id")!.toString(),'username':getCookie('username'),'func':'sync','spec':'rendermdcell'};
          comm!.send(sendx);
          break;
        case 'code':
          if (sessionContext) {
            ...
            // sessionContext.session!.kernel!.on_execute=true;
            // if(sessionContext.session!.kernel!.execute_cellid !==undefined){
            //   sessionContext.session!.kernel!.execute_cellid!.push(cell.model.metadata.get("id")!.toString());
            // }
            // if(sessionContext.session!.kernel!.multi_runcell !==undefined){
            //   sessionContext.session!.kernel!.multi_runcell!.push(cell.model.metadata.get("id")!.toString());
            // }
            // const sendx={'cellid':cell.model.metadata.get("id")!.toString(),'username':getCookie('username'),'func':'sync','spec':'executecell'};
            // comm!.send(sendx);
   ...
   */
  export function copyOrCut(notebook: Notebook, cut: boolean,comm:Kernel.IComm | null): void {
    if (!notebook.model || !notebook.activeCell) {
      return;
    }
  ...
  export function changeCellType(
    ...
    notebook.widgets.forEach((child, index) => {
      ...
      if (child.model.type !== value) {
        ...
        const sendx={'cellid':newCell.metadata.get('id')!.toString(),'celltype':value,"index":index,'func':'sync','spec':'celltype'};
        comm!.send(sendx);
  ...
  export function changecelltypebyid(notebook: Notebook, value: string,id:string,index:number) {
    const model = notebook.model!;
    const cells = model.cells;
    let cellx = notebook.widgets.find(x => x.model.metadata.get('id')!.toString() === id);
    if (cellx!.model.type.toString() !== value) {
      const cell = cellx!.model.toJSON();
      let newCell: ICellModel;
    ...
    if (value === 'markdown') {
      // Fetch the new widget and unrender it.
      cellx = notebook.widgets[index];
      (cellx as MarkdownCell).rendered = false;
    }
    notebook.deselectAll();
  }
  ...
  export function deleteCells(notebook: Notebook,comm:Kernel.IComm | null): void {
    ...
    if (toDelete.length > 0) {
      // Delete the cells as one undo event.
      const sendxx={'cellid':'',"indexs":toDelete,'func':'sync','spec':'deletecell'};
      comm!.send(sendxx);
      cells.beginCompoundOperation();
      ...
      if (!cells.length) {
        ...
        // const sendx={'cellid':id,'index':0,'func':'sync','spec':'createcell'};
        // comm!.send(sendx)
      }
      cells.endCompoundOperation();
