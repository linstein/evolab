...
// import * as Kernel from "../../services/src/kernel/kernel";
// import {IKernelConnection} from "../../services/src/kernel/kernel";
// import {Kernel} from "../../services/src/kernel";
...
export class NotebookTools extends Widget implements INotebookTools {
  ...
  constructor(options: NotebookTools.IOptions) {
    ...
    // this.kernel=this.activeNotebookPanel!.sessionContext.session!.kernel;
    // this.comm=this.kernel.createComm('kernels');
  }
  ...
  private _onActiveCellChanged(): void {
    console.log("_onActiveCellChanged=====================================")
...
export namespace NotebookTools {
  ...
  export class ActiveCellTool extends Tool {
    ...
    constructor() {
      ...
      this.layout = new PanelLayout();
      // this.comm=null;
      // this.kernel=this.notebookTools!.activeNotebookPanel!.sessionContext.session!.kernel;
      // if(!this.kernel){
      //   throw new Error('Session has no kernel.');
      // }
      // this.comm = this.kernel.createComm('test1');
      // this.comm.open()
    }
    // private kernel:IKernelConnection |null;
    // private comm:Kernel.IComm |null;
    /**
    ...
    //     this.kernel._commid=comm_id;
    //     this.comm = this.kernel.createComm('test1',comm_id);
    //     this.comm.open();
    //     this.comm.onMsg = async msg => {
    //       const cellid=msg.content.data.cellid;
    //       console.log(msg.content.data.cellid);
    //       console.log(this._cellModel!.value.text);
    //
    //       const cell=this.notebookTools.activeNotebookPanel?.content.widgets.find(x => x.model.metadata.get('id')!.toString() === cellid);
    //       if(cell){
    //         console.log('find cell by id !!!!!!!!!!!!!!');
    //         cell!.model.value.text=msg.content.data.txt!.toString();
    //       }
    //     };
    //
    //   }else{
     ...
     */
    private async _onValueChanged() {
      // this.kernel=this.notebookTools.activeNotebookPanel!.sessionContext.session!.kernel;
      // if(!this.kernel){
      //   throw new Error('Session has no kernel.');
      // }
      // const comm = this.kernel.createComm('test1');
      // comm.open();
      // this.get_comm();
      if(this.notebookTools.activeCell !==null){
        if(this.notebookTools.activeCell!.editor.hasFocus() && this.notebookTools.activeCell!.readOnly==false){
        console.log(this.notebookTools.activeCell!.model.metadata.toJSON());
        const txt=this._cellModel!.value.text;
        console.log(this.notebookTools.activeCell!.model.id!);
        const sendx={'cellid':this.notebookTools.activeCell!.model.metadata.get('id')!.toString(),'txt':txt,'func':'sync','spec':'txt'};
        if(this.notebookTools!.activeNotebookPanel!.comm !==null){
          this.notebookTools!.activeNotebookPanel!.comm!.send(sendx);
        }else{
          console.log('no comm')
        }
