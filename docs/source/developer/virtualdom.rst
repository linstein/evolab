...
widgets using the ``ReactWidget`` class from ``@evolab/apputils``:
...
To have your React element change in response to a signal event, use the ``UseSignal`` component from ``@evolab/apputils``,
