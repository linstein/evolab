...
export namespace StaticNotebook {
  ...
  export class ContentFactory extends Cell.ContentFactory
    ...
    createCodeCell(
      ...
      console.log('notebook create cell');
...
export class Notebook extends StaticNotebook {
  ...
  _dragcellMsg: (from:number,to:number) => void | PromiseLike<void>;

  set dragcellMsg(cb: (from:number,to:number) => void | PromiseLike<void>){
    console.log("set onexecmsg33333333333333333333333333333333");
    this._dragcellMsg = cb;
  }
  get dragcellMsg(): (from:number,to:number) => void | PromiseLike<void>{
    return this._dragcellMsg;
  }

  _focusMsg: (cellid:string,focus:boolean) => void | PromiseLike<void>;

  set focusMsg(cb: (cellid:string,focus:boolean) => void | PromiseLike<void>){
    console.log("set onexecmsg33333333333333333333333333333333");
    this._focusMsg = cb;
  }
  get focusMsg(): (cellid:string,focus:boolean) => void | PromiseLike<void>{
    return this._focusMsg;
  }
  ...
  select(widget: Cell): void {
    console.log('notebook cell change!!!');
  ...
  protected onUpdateRequest(msg: Message): void {
    ...
    each(this.widgets, widget => {
      ...
      if (this.isSelectedOrActive(widget)) {
        console.debug('select cell notebook');
    ...
    if (source === this) {
      ...
      if (fromIndex < toIndex) {
        each(toMove, cellWidget => {
          ...
          this.dragcellMsg(fromIndex,toIndex)
        });
        ...
        each(toMove, cellWidget => {
          ...
          //send(msg)
          console.log("move up");
          this.dragcellMsg(fromIndex-1,toIndex-1)
        });
      }
  ...
  private _evtFocusIn(event: MouseEvent): void {
    ...
    if (index !== -1) {
      ...
      if (node.contains(target)) {
        ...
        const focuscell=this.widgets[index];
        this.focuscellid=focuscell.model.metadata.get('id')!.toString();
        if (focuscell.readOnly==false){
          this.focusMsg(this.focuscellid,true)
        }
      }
    } else {
  ...
  private _evtFocusOut(event: MouseEvent): void {
    ...
    console.log("cell unfocus *****************************************");
    this.focusMsg(this.focuscellid,false);
    ...
    if (index !== -1) {
      ...
      if (widget.editorWidget.node.contains(relatedTarget)) {
        console.log("enter another cellllllllll")
        return;
      }
    }
